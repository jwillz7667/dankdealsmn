/**
 * Seed — ports the canonical catalog from design/project/js/data.js into Postgres.
 * Idempotent: safe to run repeatedly (upserts by slug / unique key).
 *
 *   pnpm db:seed
 *
...
const slugify = (s: string): string =>
  s
    .toLowerCase()
    .normalize('NFKD')
    .replace(/[^\w\s-]/g, ' ') // drop punctuation (—, :, ½, ¼, ·, parens)
    .replace(/[\s_]+/g, '-')
    .replace(/-+/g, '-')
    .replace(/^-|-$/g, '');
...
const STRAIN = {
  ...
  Hybrid: 'HYBRID',
  CBD: 'CBD',
} as const satisfies Record<string, StrainType>;

const toBadge = (b: string): Badge => b.toUpperCase() as Badge;

// ---------- source data (ported verbatim from data.js) ----------
type RawProduct = {
  id: string;
  name: string;
  brand: string;
  cat: string;
  type: keyof typeof STRAIN;
  thc: number;
  cbd: number;
  price: number;
  old?: number;
...
const ZONES = [
  'Minneapolis', 'St. Paul', 'Bloomington', 'Edina', 'Richfield', 'St. Louis Park',
  'Roseville', 'Maplewood', 'Eagan', 'Plymouth', 'Minnetonka', 'Brooklyn Park',
  'Eden Prairie', 'Burnsville',
];

async function main(): Promise<void> {
  // Store settings (singleton) — mirrors DD_DELIVERY.
  await prisma.storeSettings.upsert({
    ...
    create: {
      ...
      supportPhone: '(612) 555-0420',
      announcement: 'Free delivery on orders over $75 · Cash or debit on arrival',
    },
    update: {},
  });

  // Promo codes (DD15 / FREEDROP from store.js totals()).
  await prisma.promoCode.upsert({
    where: { code: 'DANK15' },
    create: { code: 'DANK15', type: 'PERCENT_OFF', value: 1500, description: '15% off your subtotal', isActive: true },
    update: {},
  });
  await prisma.promoCode.upsert({
    where: { code: 'FREEDROP' },
    create: { code: 'FREEDROP', type: 'FREE_DELIVERY', value: 0, description: 'Free delivery', isActive: true },
    update: {},
  });

  // Delivery zones.
  for (const [i, city] of ZONES.entries()) {
    await prisma.deliveryZone.upsert({
      where: { city },
      create: { city, sortOrder: i, isActive: true },
      update: { sortOrder: i },
    });
  }
  ...
  for (const [i, c] of CATEGORIES.entries()) {
    const row = await prisma.category.upsert({
      where: { slug: c.id },
      create: { slug: c.id, name: c.name, blurb: c.blurb, iconKey: c.icon, sortOrder: i },
      update: { name: c.name, blurb: c.blurb, iconKey: c.icon, sortOrder: i },
    });
  ...
  for (const [i, p] of PRODUCTS.entries()) {
    ...
    if (!categoryId) throw new Error(`Missing category for product ${p.id}: ${p.cat}`);

    const unit = p.cat === 'edibles' ? 'MG' : 'PERCENT';
    ...
    await prisma.product.upsert({
      ...
      create: {
        ...
        categoryId,
        brandId,
        strainType: STRAIN[p.type],
        thcValue: p.thc,
        thcUnit: unit,
        cbdValue: p.cbd,
        cbdUnit: unit,
        priceCents: Math.round(p.price * 100),
        compareAtCents: p.old ? Math.round(p.old * 100) : null,
        size: p.size,
        ...
        status: 'ACTIVE',
        isFeatured: p.badges.includes('best'),
        ratingAvg: p.rating,
  ...
  // A demo driver to match the confirmation screen.
  const existingDriver = await prisma.driver.findFirst({ where: { displayName: 'Marcus D.' } });
  if (!existingDriver) {
    await prisma.driver.create({
      data: { displayName: 'Marcus D.', vehicleDescription: 'Green hatchback', ratingAvg: 4.9 },
    });
  }

  // Bootstrap admin: the account that signs in with this email is promoted to ADMIN.
  const adminEmail = process.env.SEED_ADMIN_EMAIL ?? 'jwillz7667@gmail.com';
  await prisma.user.upsert({
    where: { email: adminEmail },
    create: { email: adminEmail, name: 'DankDeals Admin', role: 'ADMIN' },
    update: { role: 'ADMIN' },
  });

  console.warn(
    `Seed complete: ${CATEGORIES.length} categories, ${PRODUCTS.length} products, ${ZONES.length} zones. Admin: ${adminEmail}`,
  );
}
